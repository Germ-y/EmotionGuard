...
async def classify_context(text: str, audio_features: AudioFeatures | None = None) -> AnalysisResult:
    if settings.openai_api_key:
        return await classify_with_openai(text, audio_features)
    if settings.anthropic_api_key:
        return await classify_with_claude(text, audio_features)
